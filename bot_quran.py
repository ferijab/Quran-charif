...
# 🔧 تحميل الإعجابات - إنشاء ملف إذا لم يوجد
def load_likes():
    global LIKES_DB
    try:
...
async def show_surah(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # محاولة إرسال الصورة من مصادر متعددة
        image_urls = [
            f"{IMAGE_BASE}{surah_num}.png",
            f"https://cdn.alquran.cloud/media/image/{surah_num}.png",
            f"https://quran.com/images/surah/{surah_num}"
        ]
...
async def random_ayah(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if ayahs:
            ...
            text = (
                f"<b>سورة {data.get('name', '')} (آية {ayah.get('numberInSurah', '')})</b>\n"
                f"{ayah.get('text', '')}\n\n"
    ...
    try:
        ...
        for s in surahs:
            if (search_query.lower() in s['name'].lower() or 
                search_query.lower() in s['englishName'].lower() or
                search_query == str(s['number'])):
                results.append(s)
        ...
        if results:
            ...
            for s in results[:5]:
                btn_text = f"{s['number']}. {s['name']} ({s['englishName']})"
                keyboard.append([InlineKeyboardButton(btn_text, callback_data=f"surah_{s['number']}_0")])
                text += f"{btn_text}\n"
...
يمكنك التفاعل مع البوت من خلال الأزرار:
- تصفح السور
- تشغيل التلاوات
- الاستماع إلى أذكار
- البحث عن سور
